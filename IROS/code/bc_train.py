...
def save_model(model, iter, output_dir="bc_policy"):
    ...
    name = os.path.join(output_dir, f"bc_policy_iter{iter:05d}.pth")
...
# 4. Model Training
# ==========================================
# Add output_base to the arguments
def train_bc_model(xs, dxs, dt, steps=50000, lr=1e-4, output_base="."):
    """
    ...
    # Define the dynamic subfolder paths
    model_dir = os.path.join(output_base, "bc_policy")
    plot_dir = os.path.join(output_base, "bc_rollouts")
    ...
    for step in range(steps):
        ...
        if (step % 100) == 0 or step == steps - 1:
            ...
            x0 = xs[:, 0, :].clone().detach()
            xs_pred = simulate_bc_rollout(model, x0, nsamples, dt)

            # Pass the dynamically generated paths to your saving functions
...
if __name__ == "__main__":
    # Define the master directory for this specific run
    master_folder = "./IROS/pick/processed"

    # 1. Dynamically target the input files within the master folder
    trajectory_files = [
    ...
    # Pass the master_folder to the training function so it knows where to save intermediates
    trained_bc_model = train_bc_model(
        xs, dxs, dt, steps=10000, lr=1e-4, output_base=master_folder
    )

    # 2. Save the final weights directly inside the master folder
    final_out = os.path.join(master_folder, "trained_bc_policy_final.pth")
